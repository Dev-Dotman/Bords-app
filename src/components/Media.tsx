...
import { Trash2, ExternalLink, Video, Image as ImageIcon, Palette } from 'lucide-react'
...
export function Media({ id, url, title, description, type, position, width, height, color }: MediaType) {
  ...
  const isSelected = selectedItems.some(item => item.id === id && item.type === 'media')
  ...
  return (
    ...
    //   transition={false}
      ...
      onMouseEnter={() => { setIsHovered(true); setShowNodes(true); }}
      onMouseLeave={() => { setIsHovered(false); setShowNodes(false); }}
    >
      ...
      <div
        ...
        style={{ 
          ...
          ...(type === 'image' && color && !isSelected ? { borderColor: color } : {})
        }}
      >
        {/* Media Content */}
        <div className={`relative ${type === 'video' ? 'aspect-video' : ''} ${isDark ? 'bg-zinc-900' : 'bg-zinc-100'}`} style={type === 'image' ? { height: `${height}px` } : undefined}>
          {type === 'image' ? (
            imageError ? (
        ...
        {type === 'video' && (
          <div className={`p-4 border-t ${isDark ? 'border-zinc-700' : 'border-zinc-200'}`}>
            {title && (
              <h3 className={`font-semibold mb-1 line-clamp-2 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
                {title}
              </h3>
            )}
            {description && (
        ...
        {showColorPicker && (
          ...
          >
            <div className="text-xs font-medium text-gray-600 mb-2 text-center">Select Background Color</div>
